...
test("player is notified of awaiting a new story", async () => {
	...
	await truncateStory(game.id, 0, "player-1", 2);
...
describe("given other stories are available", () => {
		...
		test("player is notified of needing to repair a truncated story", async () => {
			...
			expect(notifierSpy.playersNotified[0]).toEqual({
				...
				notification: {
					gameId: game.id,
					playerId: "player-1",
					activity: new PlayerActivity.RepairingTruncatedStory(0, "Player 3's initial _____________", 19),
				},
			});
		});
	});
	describe("given the first story is ready to be continued", () => {
		...
		test("player is notified of needing to continue the story", async () => {
			await truncateStory(game.id, 2, "player-1", 2);
...
describe("player has already truncated their assigned story and is awaiting a new story", () => {
	beforeEach(() => {
		game.startStory("player-4", "Player 4's initial story content");
		game.startStory("player-1", "My initial story content.");
		game.truncateStory("player-1", 0, 2);
	});

	test("the player is notified when a story is truncated and assigned to them", async () => {
		game.startStory("player-3", "Player 3's initial story content");

		await truncateStory(game.id, 2, "player-4", 2);
		...
		expect(notifierSpy.playersNotified[0]).toEqual({
			...
			notification: {
				gameId: game.id,
				playerId: "player-1",
				activity: new PlayerActivity.RepairingTruncatedStory(2, "Player 3's initial _____________", 19),
			},
		});
	});
});

const truncateStory = require("../../../../doubles/usecases/truncateStory").makeTruncateStory({
	games: () => games,
